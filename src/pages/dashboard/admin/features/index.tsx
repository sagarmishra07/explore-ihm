...
import {FeaturesDTO} from "@/utils/formatters/FeaturesDTO";
...
const firebaseUrl = 'landing/Features'
...
function Features() {
    ...
    const getAllData = async () => {
        ...
        const afterFormat = formatted.map((val: any, index: any) => FeaturesDTO.receive(val, index));
        setRows(afterFormat)
        setLoading(false)
    }
    ...
    const onSubmit = async (values: any) => {
        try {
            setLoading(true);
            const formatted: any = FeaturesDTO.create(values.data);
    ...
    const items = rows.map((data: any, index: any) => (
        ...
        >
            <td>{data?.sNo}</td>
            <td className={""}>{data?.feature_name}</td>
                        ...
                        <section className={"flex justify-end"}>
                            <div>
                                <Button
                                    size={"md"}
                                    className={"my-1"}
                                    onClick={() => router.push("/dashboard/admin/features/add")}
                                >
                                    Add Features
                                </Button>
                            </div>
                                                ...
                                                <Controller
                                                    ...
                                                    render={({field}) => (
                                                        <TextInput
                                                            ...
                                                            error={errors.data?.feature_name?.message}
                                                        />
                                                    )}
                                                />
                                            </div>
