...
function Features() {
    ...
    const getAllData = async () => {
        ...
        const afterFormat = formatted.map((val: any, index: any) => FacilitiesDTO.receive(val, index));
        setRows(afterFormat)
    ...
    const deleteData = async (key: any) => {
        setLoading(true)
        await firebaseRemove('Features', key);
    ...
    const onSubmit = async (values: any) => {
        try {
            setLoading(true);
            const formatted: any = FacilitiesDTO.create(values.data);
            await firebaseUpdate('Facilities/data', values?.data?.key, formatted);
    ...
    const onUpdateDetails = async (values: any) => {
        try {
            ...
            await firebaseStore('Facilities/details', values?.details);
            showNotify('success', "Updated Successfully")
            reset();
                        ...
                        <section className={"flex justify-end"}>
                            <div>
                                <Button
                                    size={"md"}
                                    className={"my-1"}
                                    onClick={() => router.push("/dashboard/admin/facilities/add")}
                                >
                                    Add Facilities
                                </Button>
                            </div>
